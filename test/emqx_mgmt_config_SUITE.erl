...
run_protocol_cmd(_Config) ->
    ...
    emqx_mgmt_config:run(["config", "set", "mqtt.websocket_protocol_header=off", "--app=emqttd"]),
...
run_lager_cmd(_Config) ->
    emqx_mgmt_config:run(["config", "set", "log.console.level=info", "--app=emqttd"]),
    ok.

run_connection_cmd(_Config) ->
    emqx_mgmt_config:run(["config", "set", "mqtt.conn.force_gc_count=1000", "--app=emqttd"]),
    {ok, E} =  application:get_env(emqttd, conn_force_gc_count),
    ?assertEqual(1000, E).

run_broker_config(_Config) ->
    emqx_mgmt_config:run(["config", "set", "mqtt.broker.sys_interval=10", "--app=emqttd"]),
...
set_cmd({Key, _Type}) ->
    emqx_mgmt_config:run(["config", "set", string:join(["mqtt", Key], "."), "--app=emqttd"]).

set_cmd(Pre, {Key, _Type}) ->
    emqx_mgmt_config:run(["config", "set",  string:join(["mqtt", Pre, Key], "."), "--app=emqttd"]).
